...
red_urchins <- inverts_data %>% 
  mutate(region = ifelse(region == "Central_Coast", "Central", "Southern")) %>% 
  group_by(year, mpa_status, region) %>% 
  summarise(Red = mean(MESFRAAD_d)/60, 
            Red_se = std(MESFRAAD_d)/60) %>% 
...
purple_urchins <- inverts_data %>% 
  mutate(region = ifelse(region == "Central_Coast", "Central", "Southern")) %>% 
  group_by(year, mpa_status, region) %>% 
  summarise(Purple = mean(STRPURAD_d)/60, 
            Purple_se = std(STRPURAD_d)/60) %>% 
  ...
  facet_grid(vars(region)) +
  scale_color_manual(values=group.colors, name = "Protection Status") +
  ylab(bquote('Purple urchins per ' ~ m^2)) +
  xlab("Year") +
  annotate("rect", fill = "red", alpha = 0.2, 
           xmin = 2014, xmax = 2016,
           ymin = -Inf, ymax = Inf) +
...
##### Kelp #####################################################################
giant_kelp_per_region <- inverts_data %>% 
  mutate(region = ifelse(region == "Central_Coast", "Central", "Southern")) %>% 
  group_by(year, mpa_status, region) %>% 
  summarise(total = mean(MACPYRAD_d),
            total_se = std(MACPYRAD_d)) %>% 
  ggplot(aes(x = year, y = total, color = mpa_status)) +
  geom_line(linewidth = 1) +
  geom_errorbar(aes(ymin = total - total_se, 
                    ymax = total + total_se), 
...
giant_kelp_per_region

bull_kelp_per_region <- inverts_data %>% 
  mutate(region = ifelse(region == "Central_Coast", "Central", "Southern")) %>% 
  group_by(year, mpa_status, region) %>% 
  summarise(total = mean(NERLUE_d),
            total_se = std(NERLUE_d)) %>% 
  ggplot(aes(x = year, y = total, color = mpa_status)) +
  geom_line(linewidth = 1) +
  geom_errorbar(aes(ymin = total - total_se, 
                    ymax = total + total_se), 
...
bull_kelp_per_region


##### Export ###################################################################
png("Figures/Urchins_MLPA.png", width = 9, height = 6, 
    units = "in", res = 600)
combo_urchin_plot
dev.off()

png("Figures/Urchins_per_region_MLPA.png", width = 9, height = 5, 
    units = "in", res = 600)
Urchins_per_region
dev.off()

png("Figures/Sheephead_lobsters_MLPA.png", width = 6, height = 10, 
    units = "in", res = 600)
combo_plot
dev.off()

png("Figures/Macrocystis_MLPA.png", width = 9, height = 5, 
    units = "in", res = 600)
giant_kelp_per_region
dev.off()

png("Figures/Nereocystis_MLPA.png", width = 9, height = 5, 
    units = "in", res = 600)
bull_kelp_per_region
dev.off()
