...
gravity_75 <- quantile(kelp_data_all$gravity, .75)

# Join data back 
kelp_data <- kelp_data_all %>% 
  filter(region == "South_Coast" | region == "Central_Coast") %>% 
  filter(year >= 2014) %>% 
  dplyr::select(PixelID, year, mpa_status, area, gravity) %>% 
  left_join(df_percent_recovery, by = c("PixelID", "year", "area")) %>% 
  filter(gravity >= gravity_75)

mpas_id <- kelp_data %>% 
  filter(mpa_status != "None") %>% 
  select(PixelID) %>% unique() %>% 
  mutate(protected = 1)
  
points_in_mpas <- points_in_mpas %>% 
  full_join(mpas_id, by = "PixelID") %>% 
  filter(protected == 1)
...
# Author: Joy Kumagai (kumagaij@stanford.edu) 
# Purpose: Permutation analysis - defining percent recovery with a minimum of zero 
#          and considering the effect of MPA age
# BIO 202: Ecological Statistics
...
# Pixels that are within MPAs established after 2003
