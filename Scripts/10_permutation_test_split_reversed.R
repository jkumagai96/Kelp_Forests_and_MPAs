...
for (i in 1:length(years)) {
  ...
  if (length(unique(df$N_F)) == 1) {
    sig_N_F <- NA } else {
      sig_N_F <- 1 - ecdf(df$N_F)(tv$N_F)
    }
  
  if (length(unique(df$N_P)) == 1) {
    sig_N_P <- NA } else {
      sig_N_P <- 1 - ecdf(df$N_P)(tv$N_P)
    }
  
  if (length(unique(df$P_F)) == 1) {
    sig_P_F <- NA } else {
...
write.csv(final_results_south, 
          "Processed_data/data_tables/bootstrap_south_coast_reversed.csv", 
          row.names = F)
...
write.csv(final_results_central, 
          "Processed_data/data_tables/bootstrap_central_coast_reversed.csv", 
          row.names = F)
...
write.csv(final_results_north_central, 
          "Processed_data/data_tables/bootstrap_north_central_coast_reversed.csv", 
          row.names = F)
...
write.csv(final_results_north, 
          "Processed_data/data_tables/bootstrap_north_coast_reversed.csv", 
          row.names = F)