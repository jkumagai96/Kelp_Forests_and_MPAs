...
# Purpose: Permutation analysis - defining percent recovery with a minimum of zero
...
kelp_data <- kelp_data_all %>% 
  filter(region == "South_Coast" | region == "Central_Coast") %>% 
  filter(year >= 2014) %>% 
  dplyr::select(PixelID, year, mpa_status, area) %>% 
  left_join(df_percent_recovery, by = c("PixelID", "year", "area"))

write.csv(kelp_data, "Processed_data/data_tables/relative_area.csv", row.names = F)
...
write.csv(final_results, 
          "Processed_data/data_tables/bootstrap_percent_recovery_zero_as_min.csv", 
          row.names = F)
...
png("Figures/Bootstrap_percent_recovery_zero_as_min.png", width = 5, height = 3, units = "in", res = 600)
plot1
dev.off() 
...
relative_area_barplot <- kelp_data %>% 
  ...
  geom_boxplot2(width = 0.8, width.errorbar = 0.2) +
  theme_bw() +
  theme(legend.position = "none") +
  labs(y = "Relative Area", x = "Year", title = "Central and Southern California")
relative_area_barplot

# percent recovery split by region

sc_barplot <- kelp_data %>% 
  ...
  theme_bw() +
  theme(legend.position = "none") +
  labs(y = "Relative Area", x = "Year", title = "Southern California")
sc_barplot

cc_barplot <- kelp_data %>% 
  ...
  theme_bw() +
  theme(legend.position = "none") +
  labs(y = "Relative Area", x = "Year", title = "Central California")
cc_barplot

library(cowplot)
plot2 <- plot_grid(relative_area_barplot, cc_barplot, sc_barplot, labels = c('A', 'B', 'C'), nrow = 1)


###### Export ##################################################################
png("Figures/Relative_area_barplot.png", width = 12, height = 5, 
    units = "in", res = 600)
plot2 
dev.off() 
