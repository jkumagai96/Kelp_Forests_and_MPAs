...
study_years <- year[year >= 2009]
study_quarters <- quarter[year >= 2009]
...
data_quarter <- rasterToPoints(k) %>% 
  as.data.frame() %>% 
  filter(y <= 37.4) %>% 
  mutate(PixelID = PixelID) 

## Section to test which rows have zero area reported
test <- data_quarter %>% 
  dplyr::select(-x,-y,-PixelID)
...
data_quarter_long <- data_quarter %>% 
  pivot_longer(Q1_2009:Q4_2021, names_to = "Time", values_to = "value") %>% 
...
df <- data_all %>% 
  ...
  left_join(., depth_p, by = c("x", "y", "PixelID")) %>% 
  rename("lat" = "y", 
         "long" = "x") %>% 
  relocate(depth, .after = PixelID) %>% 
  mutate(year = str_sub(df$Time, start= -4),
         quarter = str_sub(df$Time, 1, 2)) %>% 
  dplyr::select(-Time)
