...
max_2014_2016 <- kelp_data_all %>% 
  ...
  dplyr::select(PixelID, year, area) %>% # CHANGED THESE TWO LINES TO mean_area instead of min_area
  pivot_wider(names_from = year, values_from = area) 
...
df$min <- apply(max_2014_2016, 1, FUN = min, na.rm = T) # Change this so that it is the min of 2014, 2015, 2016
...
write.csv(final_results, 
          "Processed_data/data_tables/bootstrap_percent_recovery_max_avg_year.csv", 
          row.names = F)
...
png("Figures/Bootstrap_percent_recovery_max_avg_year.png", width = 5, height = 3, units = "in", res = 600)
plot1
dev.off() 
