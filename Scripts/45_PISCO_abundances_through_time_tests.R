...
model_sheephead <- glmmTMB(
  SPUL_d ~ mpa_status + 
    ...
    ar1(0 + year_fct | site), # random slopes and autoregressive function 
...
model_sheephead2 <- glmmTMB(
  SPUL_d ~ mpa_status + 
    (year | site), # random intercept and slopes
  data = after_df, family = tweedie(link = "log")
)
...
car::Anova(model_sheephead3)
plot(emmeans(model_sheephead3, specs = pairwise ~ mpa_status, type = "response"))
