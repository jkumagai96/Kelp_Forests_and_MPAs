...
# Purpose: Sensitivity analysis of the Bootstrap - we will remove the 1x1km2 pixels which 
#          include 5 or less 30x30m pixels 
#          Then conduct the permutation approach based on difference in percent recovery 
...
##### Removing Pixels with 5 or less 30x30 potential kelp habitat pixels #######
# How much total area is removed? 0.0542167
# How much area after 2016 is removed? 0.06598982
total_area <- sum(kelp_data_all$area)
area_post_2016 <- kelp_data_all %>% 
  filter(year > 2016) 
total_area_post_2016 <- sum(area_post_2016$area)

kelp_data_all <- kelp_data_all %>% 
  left_join(n_30x30, by = "PixelID") %>% 
  filter(count > 5)

new_area <- sum(kelp_data_all$area)
100 - (new_area/total_area)*100

area_post_2016 <- kelp_data_all %>% 
  filter(year > 2016) 
new_area_post_2016 <- sum(area_post_2016$area)

100 - (new_area_post_2016/total_area_post_2016)*100
# How much area after 2016 is removed?
...
write.csv(final_results, 
          "Processed_data/data_tables/bootstrap_sensitivty_30x30_removal.csv", 
          row.names = F)
...
png("Figures/Bootstrap_sensitivity_30x30_removal.png", width = 5, height = 3, units = "in", res = 600)
plot1
dev.off() 
...
# box plot not seperated by mpa status 
kelp_data %>% 
  mutate(year = as.factor(year)) %>% 
  ggplot(aes(x = year, y = percent_recovery)) +
  geom_boxplot() +
...
kelp_data %>% 
  group_by(year, mpa_status) %>% 
  summarize(mean_percent_recovery = mean(percent_recovery)) %>% 
  ggplot(aes(x = year, y = mean_percent_recovery, color = mpa_status)) +
  geom_line() +
  geom_point()
...
kelp_data %>% 
  group_by(year, mpa_status) %>% 
  summarize(med_percent_recovery = median(percent_recovery)) %>% 
