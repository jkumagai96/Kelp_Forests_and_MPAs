...
# Purpose: Permutation Analysis - Sensitivity to how percent reocvery is defined 
...
max_area_2014_2016 <- kelp_data_all %>% # Now, not actually mins, but rather the average of maxes 
  filter(region == "South_Coast" | region == "Central_Coast") %>% 
...
df$min <- apply(max_area_2014_2016, 1, FUN = mean, na.rm = T) # mean of maxes 
df$max <- apply(maxes, 1, FUN = mean, na.rm = T)

# Check to see whether the mean of maxes is lower or equal to the "min" value
df[df$max <= df$min,] # 384 records that will need to be removed! 
...
write.csv(final_results, 
          "Processed_data/data_tables/bootstrap_percent_recovery_avg_of_maxes.csv", 
          row.names = F)
...
png("Figures/Bootstrap_percent_recovery_avg_of_maxes.png", width = 5, height = 3, units = "in", res = 600)
plot1
dev.off() 
